...
def main():
    ...
    for wi, label in enumerate(ordered):
        ...
        wrows.append(f'<button class="navrow" data-week="{esc(label)}"><span>{esc(label)}{latest}</span><b>{len(res)}</b></button>')
        cards = []
        ...
        sections.append(
            ...
            f'<div class="weekhead"><span class="weeknum">{wnum}</span><h2>{esc(label)}</h2>{badge}'
    ...
    share_text = (f"Useful things worth opening: {total} hand-curated AI tools, dev tools and repos "
                  f"from CheapInfra's #share-tech, via @{X_HANDLE}")
...
<meta property="og:site_name" content="CheapInfra #share-tech Resource Library">
...
<meta name="twitter:title" content="Resource Library — CheapInfra #share-tech">
  ...
  <header class="hero">
    ...
    <div class="hero-grid">
      ...
      <aside class="review" aria-label="Review summary">
        ...
        <span class="rsub">Across {len(ordered)} weekly drops, with boundary-day catch-ups folded in.</span>
      </aside>
    </div>
  </header>
  <div class="layout">
...
{"".join(sections)}
  ...
  <section id="analytics" hidden>
    <div class="mainhead"><h2 class="shortlist-h">Analytics</h2><span class="mcount">{total} resources · {len(ordered)} weeks</span></div>
