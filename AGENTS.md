...
| `feed.xml` | RSS 2.0 feed of all resources, newest first. Generated by `build.py`. |
| `og-image.png` | Social card (1200×630, black editorial theme), regenerated by `build.py` with the current count. |
| `scripts/linkcheck.py` | Link-rot checker: HEAD-checks every URL in `data.json`. Run monthly; exit 1 if any dead. |
